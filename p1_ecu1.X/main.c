 ...
 * File:   main.c
 * Author: sakthi
 * Can based automotive dashboard - ECU1 - RPM and Indicator Status 
 * Created on December 3, 2025, 5:19 PM
 */
...
// LEDS
#define MAKE_PORTB_OUTPUT TRISB = (TRISB&0x7E)
#define LED1    RB0
#define LED2    RB7
#define LEFT    0
#define RIGHT   1
...
#define RPM_ADC_CHANNEL   CHANNEL4
...
// Message IDs
#define RPM_MSG_ID    0x36E
#define IND_MSG_ID     0x36E
...
uint16_t get_rpm(void)
{
    uint16_t rpm;
    uint32_t adc_sum = 0;
    unsigned char samples = 10;  // Taking 10 samples for averaging
    unsigned char i;
    
    /* Take multiple ADC readings and average them */
    for (i = 0; i < samples; i++)
    {
        adc_sum += read_adc(RPM_ADC_CHANNEL);
        __delay_us(100);  // Small delay between samples
    }
    
    /* Calculate average */
    rpm = adc_sum / samples;
    rpm = (rpm/10.23)*60;   // Scaling to 0-6000
    
    return rpm;
}

unsigned char get_indicator_sts(void)
{
    ...
    if (key != 0xFF)
    {
        if (key == MK_SW1)   // Left - Switch 1
        {   
            ind = 1;
        }
        else if (key == MK_SW2)  // Right - Switch 2
        {
            ind = 2;
        }
        else if (key == MK_SW3)  // hazard - Switch 3
        {
            ind = 3;
        }
        else if (key == MK_SW4)  // off - Switch 4
        {
            ind = 0;
        }
    }
...
void main(void)
{
    ...
    while (1)
    {
        ...
        /* Get indicator status from keypad */
        ind = get_indicator_sts();
        
        /* Prepare rpm data for transmission */
        sprintf((char*)buf, "%04u", rpm);
        ...
        /* Prepare indicator data for transmission */
